...
function BibtexDisplay() {
    ...
    this.getName = function(array) {
        ...
        if (name[0] != "") {
            name[4] = name[0].split(" ").map(function(s){return s.substring(0, 1).toUpperCase()}).join(". ") + ".";
            name[5] = name[4]+" "+name[3];
        }
        return name;
    }
    ...
    this.displayAuthor = function(string, format) {
		...
		if (name[3].toUpperCase()+', '+name[0].toUpperCase() == corresponding_author)
			name[index] = "<i><b><u>"+name[index]+'</u></b></i>'
		...
		if (arrayString[i].toUpperCase() == corresponding_author)
			arrayString[i] = "<i><b><u>"+arrayString[i]+'</u></b></i>'
	    }
...
function bibtex_js_draw() {
    ...
    if ($(".corresponding_author").length) {
        corresponding_author = $(".corresponding_author")[0].getAttribute("extra").toUpperCase();
    }
    if ($("#bibtex_input").length) {
        ...
        $('bibtex').each(function(index, value) {
            var request = $.ajax({
                ...
                .done(function(data){return bibstring += data})
                .fail(function(request, status, error) {return console.error(error)})
